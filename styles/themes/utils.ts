...
const localThemeData = JSON.parse(
	globalThis.localStorage?.getItem?.(THEME_STORAGE_NAME)! ?? null
)
	...
	subscribeWithSelector(
		persist(
			(set, get) => ({
				...
				recoverTheme: () =>
					set({ theme: get().theme, mode: get().mode, radius: get().radius }),
...
const recoverTheme = (
	theme: ThemeName = initialTheme,
	mode: ModeName = initialMode,
...
export { recoverTheme, useThemeStore }
