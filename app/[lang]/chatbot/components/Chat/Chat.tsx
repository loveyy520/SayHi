		...
		return (
			<section className='relative flex-1 overflow-hidden bg-background '>
				{modelError ? (
					...
					<>
						<div
							className='custom-scrollbar max-h-full pt-9 overflow-x-hidden bg-background'
							ref={chatContainerRef}
							onScroll={handleScroll}
						>
							{conversation.messages.length === 0 ? (
								<>
									<section className='mx-auto flex w-full flex-col space-y-10 pt-12 sm:max-w-[560px]'>
								...
								<>
									<Card className='flex absolute w-full h-12 top-0 z-10 justify-center items-center border border-border bg-background py-2 text-sm text-foreground dark:border-none'>
                                                {dictionary.chat['Model']}: {conversation.model.name}
										<Popover>
										...
										<AlertDialog>
											...
											<AlertDialogContent>
												<AlertDialogHeader>
													<AlertDialogTitle>
                                                                {dictionary.chat['Are you sure you want to clear all messages?']!}
													</AlertDialogTitle>
													<AlertDialogDescription>
														{
                                                                    dictionary.chat[
												...
												<AlertDialogFooter>
                                                            <AlertDialogCancel>{dictionary.chat['Cancel']!}</AlertDialogCancel>
													<AlertDialogAction onClick={onClearAll}>
                                                                {dictionary.chat['Continue']}
													</AlertDialogAction>
												</AlertDialogFooter>
											</AlertDialogContent>
										</AlertDialog>
				...
				{showScrollDownButton && (
					<div className='absolute bottom-52 right-0 mb-4 mr-4 pb-20'>
						...
						>
							<i className='i-[material-symbols--arrow-circle-down-rounded] text-primary animate-bounce text-lg'></i>
						</Button>
					</div>
				)}
			</section>
